...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
        ...
        for nemesis in list_of_nemesis:
            ...
            for filter_name, filter_value in flags.items():
                ...
                try:
                    ...
                    self.log.warning("The required nemesis flag was not found")
    ...
    def build_list_of_disruptions_to_execute(self, nemesis_filter=None, nemesis_multiply_factor=1):
        """
        Builds the list of disruptions that should be excuted during a test.

        nemesis_filter: should be retrived from the test yaml by using the "nemesis_filter".
        Here it kept for future usages and unit testing ability.
        more about nemesis_filter behaviour in sct_config.py

        nemesis_multiply_factor: should be retrived from the test yaml by using the "nemesis_multiply_factor".
        Here it kept for future usages and unit testing ability.
        more about nemesis_filter behaviour in sct_config.py
        """
        nemesis_filter = self.cluster.params.get('nemesis_filter')
        if nemesis_filter:
            subclasses = self.get_list_of_subclasses_by_property_name(list_of_properties_to_include=nemesis_filter)
            if subclasses:
