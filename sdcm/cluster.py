...
class BaseNode():  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def __init__(self, name, parent_cluster, ssh_login_info=None, base_logdir=None, node_prefix=None, dc_idx=0):  # pylint: disable=too-many-arguments,unused-argument
        ...
        # enable bootstrap.
        self.enable_auto_bootstrap = Setup.AUTO_BOOTSTRAP
        self.scylla_version = ''
...
class BaseScyllaCluster:  # pylint: disable=too-many-public-methods
    ...
    def seed_nodes_ips(self):
        _seed_nodes_ips = [node.ip_address for node in self.nodes if node.is_seed]

        assert _seed_nodes_ips, "We should have at least one selected seed by now"
        return _seed_nodes_ips

    @property
    def seed_nodes(self):
        _seed_nodes = [node for node in self.nodes if node.is_seed]
        assert _seed_nodes, "We should have at least one selected seed by now"
        return _seed_nodes
    ...
    @retrying(n=60, sleep_time=30, allowed_exceptions=(ClusterNodesNotReady, UnexpectedExit),
...
class BaseMonitorSet():  # pylint: disable=too-many-public-methods,too-many-instance-attributes
    ...
    def start_scylla_monitoring(self, node):
        ...
        run_script = dedent(f"""
            ...
            -d `realpath "{self.monitoring_data_dir}"` -l -v master,{self.monitoring_version} -b "-web.enable-admin-api" -b "-query.max-samples=150000000"
        """)
