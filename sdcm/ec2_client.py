...
class EC2ClientWrapper:
    ...
    def _request_spot_instance(  # noqa: PLR0913
        self,
        instance_type,
        image_id,
        region_name,
    ...
    def _is_fleet_request_fulfilled(self, request_id):
        ...
        for req in resp["SpotFleetRequestConfigs"]:
            if (
                ...
                or "ActivityStatus" not in req
                or req["ActivityStatus"] != STATUS_FULFILLED
            ):
                if "ActivityStatus" in req and req["ActivityStatus"] == SPOT_STATUS_UNEXPECTED_ERROR:
                    ...
                    resp = self._client.describe_spot_fleet_request_history(
                        SpotFleetRequestId=request_id, StartTime=search_start_time, MaxResults=10
                    )
    ...
    @retrying(n=5, sleep_time=10, allowed_exceptions=(ClientError,), message="Waiting for instance is available")
    def add_tags(self, instance_ids: list[str] | list[Instance] | str | Instance, tags: dict = None) -> None:
        ...
        instance_ids = [
            instance_or_id if isinstance(instance_or_id, str) else instance_or_id.instance_id
            for instance_or_id in instance_ids
        ]
    ...
    def create_spot_instances(
        ...
        request_ids = self._request_spot_instance(
            instance_type,
            image_id,
            region_name,
            network_if,
    ...
    def create_spot_fleet(
        ...
        if not instance_ids:
            err_code = (
                resp
                if resp in [FLEET_LIMIT_EXCEEDED_ERROR, SPOT_CAPACITY_NOT_AVAILABLE_ERROR]
                else SPOT_STATUS_UNEXPECTED_ERROR
            )
            raise CreateSpotFleetError(
                error_response={"Error": {"Code": err_code, "Message": resp}}, operation_name="create_spot_fleet"
            )
    ...
    def get_instance_by_private_ip(self, private_ip):
        ...
        :return: EC2.Instance object
        """
        instances = self._client.describe_instances(Filters=[{"Name": "private-ip-address", "Values": [private_ip]}])
    ...
    def add_placement_group_name_param(self, boto3_params, placement_group_name):
        if placement_group_name:
            ...
            list_placement_groups = list_placement_groups_aws(placement_group_tags, self.region_name)
            if list_placement_groups:
                ...
                error_message = (
                    f"use_placement_group param is true but no placement group with tags {placement_group_tags} found"
                )
                raise GetPlacementGroupError(error_message)
    ...
    def add_host_id_param(boto3_params, availability_zone):
        if host_id := SCTDedicatedHosts.get_host(availability_zone, boto3_params["InstanceType"]):
            boto3_params["Placement"] = {**boto3_params.get("Placement", {}), "HostId": host_id}
